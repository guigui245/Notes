...
\definition{
    Let $E \rightarrow M$ be a smooth vector bundle, a \textbf{connection} is a vector bundle morphism $\nabla: E \rightarrow DE \subset \mathrm{End}(E)$ to its derivation bundle covering the identity.
    % 
    \footnote{
        Derivation bundle will be studied more in \ref{sub:derivations}
    }
    % 
...
\definition{
    A \textit{A-connection} $~^A\nabla$ on a Lie algebroid $A \rightarrow M$ is an anchor compatible \textit{Kozul connection} on the vector bundle $A \rightarrow M$. \citep{Kotov2019}
    So it is a bundle morphism $\nabla : A \rightarrow DA$ covering the identity\footnotemark . 
    Simply put, this is a $C^\infty(M)$-module with values in regular differential operators of order $1$ on M
    \begin{align*}
