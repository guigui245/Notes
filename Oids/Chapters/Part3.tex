...
\definition{
    ...
    \end{equation}
    On an open set $U \subset M$, a basis of sections $\{e_i : U \rightarrow \pi^{-1}(U) \vert \pi \circ e_i = \mathrm{id}_U\}_{i=1}^{\mathrm{rk}(E)}$ defines a local trivialisation (\cref{def:local trivialisation}). 
...
\definition{
    ...
    \end{align}
    Locally, if the algebroid structure is given by structure functions $\tensor{C}{_{ab}^c}(x) \in \Gamma( \Lambda^2 A^* \otimes A)$ in a chart $U_\alpha$ with coordinates $x^i$, we deduce from the above a Jacobi type identity:
    \begin{equation}
...
% 
\remark{
    In general, vector bundle morphism \textbf{DO NOT} induce well-defined maps between modules of sections. 
    This is discussed further in \citep{Wright2019} by introducing the notion of double pullback algebroid and probably needs to be understood further here.
}\label{rem:double pullback lie algebroid}
% 
\begin{tcolorbox}
    could this be related to \cref{rem:duality_not_functor} ?
\end{tcolorbox}
\remark{
