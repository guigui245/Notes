...
% 
\definition{
    If a short exact sequence of $H$-modules as above splits, then we call $(\mathfrak{g},\mathfrak{h})$ \textbf{reductive} and define the orthogonal complement $\mathfrak{m} \subseteq \mathfrak{g}$ as the $H$-module such that
    \begin{align}
        \mathfrak{g} = \mathfrak{h} \oplus \mathfrak{m}
    \end{align}
    A consequence of a splitting is 
    \begin{equation}
        [\mathfrak{h}, \mathfrak{m}] \subseteq \mathfrak{m}. 
    \end{equation}
    The reductive case will always be denoted by green throughout the rest of this lecture.
}\label{def:reductive}
% 
\definition{
    A pair of \textit{reductive} Lie algebras $(\mathfrak{g,h})$ is called \textbf{symmetric} if it further follows the condition
    \begin{equation}
